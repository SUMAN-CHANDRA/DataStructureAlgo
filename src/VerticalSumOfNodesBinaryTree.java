...
public class VerticalSumOfNodesBinaryTree { 
...
//		t.VerticalSumMain(); 
	...
	static void printPreorder(TreeNode node,int val) 
    { 
        if (node == null) 
            return; 
        ...
        if(sum.get(val)!=null)
        {
        	sum.put(val,sum.get(val)+node.key());
        }
        else
        {
        	sum.put(val,node.key());
        }
  
        /* then recur on left sutree */
        printPreorder(node.left(),val-1); 
  
        /* now recur on right subtree */
        printPreorder(node.right(),val+1); 
    } 
} 
